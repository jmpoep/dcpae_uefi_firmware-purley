...
//
  ...
  if (PciDevice->BusNumber == DEFAULT_PCI_BUS_NUMBER_PCH &&
      PciDevice->DeviceNumber == PCI_DEVICE_NUMBER_EVA   &&
      ( PciDevice->FunctionNumber == PCI_FUNCTION_NUMBER_EVA_MROM0 || PciDevice->FunctionNumber == PCI_FUNCTION_NUMBER_EVA_MROM1 )
    ...
    )
  {
    ...
    Status = EFI_SUCCESS;
    MromHookEnable = 0;
    if (PciDevice->FunctionNumber == PCI_FUNCTION_NUMBER_EVA_MROM0) {
    ...
    } else if (PciDevice->FunctionNumber == PCI_FUNCTION_NUMBER_EVA_MROM1) {
    ...
    if (!EFI_ERROR(Status)) {
      if (MromHookEnable == 1) {
        DEBUG(( EFI_D_INFO, "   LoadOpRomImage() MROM Hook\n"));
        return EFI_SUCCESS;
      }
    }
  }
...
//
  ...
  Bus             = PciDevice->BusNumber;
  Device          = PciDevice->DeviceNumber;
  Function        = PciDevice->FunctionNumber;

  PciRootBridgeIo = PciDevice->PciRootBridgeIo;
  Address = EFI_PCI_ADDRESS (Bus, Device, Function, PCI_CLASSCODE_OFFSET+2);
  ...
  Status = GetSpecificConfigGuid (&gEfiSetupVariableGuid, &SysCfg);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }
  //
  // Skip loading network driver if "EFI Network" setup option is disabled.
  //
  if ((SysCfg.EfiNetworkSupport == 0) && (Class == PCI_CLASS_NETWORK) ) {
    return EFI_NOT_FOUND;
  }
//
// PURLEY_OVERRIDE_END
