...
//
  PCI_IO_DEVICE *OnBoardVideoPciRP = NULL;
  UINT16        Data16 = 0;
  ...
  if (IS_PCI_VGA (&PciIoDevice->Pci)) {
    //
    // Make sure this vga has the Root Port.
    //
    if (PciIoDevice->BusNumber != 0) {
      OnBoardVideoPciRP = PciIoDevice->Parent;
    }
  }
//
// PURLEY_OVERRIDE_END
//
  ...
  if (Operation == EfiPciIoAttributeOperationEnable) {
    ...
    //
    // If the device is offboard video and onboard video has been found
    // disable onboard video VGA control
    //
...
// PURLEY_OVERRIDE_BEGIN
//
      if (OnBoardVideoPciRP != NULL) {
        OnBoardVideoPciRP->PciIo.Pci.Read (
                                       &OnBoardVideoPciRP->PciIo, 
                                       EfiPciIoWidthUint16, 
                                       PCI_BRIDGE_CONTROL_REGISTER_OFFSET,
                                       1,
                                       &Data16
                                       );
       if ((Data16) & EFI_PCI_BRIDGE_CONTROL_VGA | EFI_PCI_BRIDGE_CONTROL_VGA_16) {
         Data16 &= ~(EFI_PCI_BRIDGE_CONTROL_VGA | EFI_PCI_BRIDGE_CONTROL_VGA_16);
         OnBoardVideoPciRP->PciIo.Pci.Write (
                                        &OnBoardVideoPciRP->PciIo, 
                                        EfiPciIoWidthUint16, 
                                        PCI_BRIDGE_CONTROL_REGISTER_OFFSET,
                                        1,
