...
// PURLEY_OVERRIDE_BEGIN
//
#define PCI_BUS_NUMBER_ME_KT      0
#define PCI_DEVICE_NUMBER_ME_KT   0x18
#define PCI_FUNCTION_NUMBER_ME_KT 0x3

#define PCI_BUS_NUMBER_IE_KT        0
#define PCI_DEVICE_NUMBER_IE_KT     0x1B
#define PCI_FUNCTION_NUMBER_IE_KT   0x3
//
// PURLEY_OVERRIDE_END
//
...
//
  //
  // For SVOS the requirement for MROM PCIe device is to give the MROM
  // a valid BAR in EROMBAR register.
  // For customer the functionality would be like any other OpROM, a
  // valid address is given to EROMBAR only to perform the OpROM
  // memory copy. After that OpROM will be executed from memory and
  // the EROMBAR will be cleared.
  //
  if (Bus == DEFAULT_PCI_BUS_NUMBER_PCH &&
      Device == PCI_DEVICE_NUMBER_EVA   &&
      (Func == PCI_FUNCTION_NUMBER_EVA_MROM0 || Func == PCI_FUNCTION_NUMBER_EVA_MROM1)
#if defined(ME_SUPPORT_FLAG) && ME_SUPPORT_FLAG
      && MeTypeIsDfx() // Activate MROM hook only if it is DFX ME
#endif // ME_SUPPORT_FLAG
      )
  {
    UINT8  MromHookEnable;
    EFI_STATUS Status;
    Status = EFI_SUCCESS;
    MromHookEnable = 0;

    if (Func == PCI_FUNCTION_NUMBER_EVA_MROM0) {
      Status = GetOptionData(&gEfiPchRcVariableGuid, OFFSET_OF (PCH_RC_CONFIGURATION, PchEvaMrom0HookEnable), &MromHookEnable, sizeof(MromHookEnable));
    } else if (Func == PCI_FUNCTION_NUMBER_EVA_MROM1) {
      Status = GetOptionData(&gEfiPchRcVariableGuid, OFFSET_OF (PCH_RC_CONFIGURATION, PchEvaMrom1HookEnable), &MromHookEnable, sizeof(MromHookEnable));
    }
    if (!EFI_ERROR(Status)) {
      if (MromHookEnable == 1) {
        Offset = PciParseBar (PciIoDevice, PCI_EXPANSION_ROM_BASE, 0x0, TRUE);
        DEBUG (( EFI_D_INFO,"   GatherDeviceInfo() MROM Hook\n"));
      }
    }
  } else {
  //
  ...
  for (Offset = 0x10, BarIndex = 0; Offset <= 0x24 && BarIndex < PCI_MAX_BAR; BarIndex++) {
    ...
    Offset = PciParseBar (PciIoDevice, Offset, BarIndex, FALSE);
  }
  }
//
// PURLEY_OVERRIDE_END
...
// PURLEY_OVERRIDE_BEGIN
//
  if (PciParseBar (PciIoDevice, 0x10, PPB_BAR_0, FALSE) == 0x14) {
    //
    // Not 64-bit bar
    //
    PciParseBar (PciIoDevice, 0x14, PPB_BAR_1, FALSE);
  }
//
// PURLEY_OVERRIDE_END
//
...
// PURLEY_OVERRIDE_BEGIN
//
  PciParseBar (PciIoDevice, 0x10, P2C_BAR_0, FALSE);
//
// PURLEY_OVERRIDE_END
//
...
// PURLEY_OVERRIDE_BEGIN
//
  if (ForceBarAssgnment == TRUE) {
    Status = EFI_SUCCESS;
    Value &= 0xFFFFFFFE; // Clear Enable bit (BIT0) to force BAR assignment.
  }
//
// PURLEY_OVERRIDE_END
//
...
//
  /* Silicon WA,
        HSD-ES: 1404253832
        HSD: 5330499
     Due to the bug described in HSD-ES: 1403992269, HSD: 5330499,
     ME and IE's KT IO bar must be aligned to 16 bytes.
   */

    if ((PciIoDevice->BusNumber == PCI_BUS_NUMBER_ME_KT &&
      PciIoDevice->DeviceNumber == PCI_DEVICE_NUMBER_ME_KT &&
      PciIoDevice->FunctionNumber == PCI_FUNCTION_NUMBER_ME_KT) ||
      (PciIoDevice->BusNumber == PCI_BUS_NUMBER_IE_KT &&
      PciIoDevice->DeviceNumber == PCI_DEVICE_NUMBER_IE_KT &&
      PciIoDevice->FunctionNumber == PCI_FUNCTION_NUMBER_IE_KT)) {

      DEBUG((EFI_D_INFO, "[WA][kt-io-bar_alignment] before WA Value = %p\n", Value));
      Value &= ~(0x0000000c);
      DEBUG((EFI_D_INFO, "[WA][kt-io-bar_alignment] after WA Value  = %p\n", Value));
    }
//
// PURLEY_OVERRIDE_END
//
