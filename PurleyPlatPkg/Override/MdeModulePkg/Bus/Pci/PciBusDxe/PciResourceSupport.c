...
//
//OVERRIDE_HSD_5332995_BEGIN
//
struct PCI32BitBarDevices PCI32BitBarDevicesTable[] = { 
  // VID, DID
  {0x8086, 0x2260}      //Intel KNL Cougar Rock (Pcie Co-Processor)
};
//
//OVERRIDE_HSD_5332995_END
//
//
...
//
      ...
      for (Count = 0; Count < sizeof(PCI32BitBarDevicesTable)/sizeof(struct PCI32BitBarDevices); Count++) {
        if (PCI32BitBarDevicesTable[Count].Vid == PciIoDevice->Pci.Hdr.VendorId && PCI32BitBarDevicesTable[Count].Did == PciIoDevice->Pci.Hdr.DeviceId) {
          //Only some special PCIE devices in the table not allowed to degrade resources
          BarFlag = 0;
          break;
        }
      }
      DegradeUsbBar = 0;
      if (PciIoDevice->BusNumber == DEFAULT_PCI_BUS_NUMBER_PCH &&
          PciIoDevice->DeviceNumber == PCI_DEVICE_NUMBER_PCH_XHCI &&
          PciIoDevice->FunctionNumber == PCI_FUNCTION_NUMBER_PCH_XHCI) {
        GetOptionData (&gEfiPchRcVariableGuid, OFFSET_OF (PCH_RC_CONFIGURATION, PchUsbDegradeBar), &DegradeUsbBar, sizeof (DegradeUsbBar));
        if (DegradeUsbBar == 1) {
          DEBUG ((EFI_D_INFO, "Degrading XHCI BAR to 32-bits.\n"));
        }
      }

      if ((PciIoDevice->RomSize != 0 && BarFlag == DOWN_TO_BELOW_4G_MMIO) || DegradeUsbBar) {
//
//OVERRIDE_HSD_5332995_END
//
