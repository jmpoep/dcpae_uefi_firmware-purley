...
//
/**
  Checks if the given memory range is mapped as requested by the MTRRs and default memory type
...
{
  UINT32  Index;
  BOOLEAN ConflictingAttributeFound = FALSE;
  
  if (Start < SIZE_1MB) {
    //
    // Variable MTRRs start at 1MB
    //
    Start = SIZE_1MB;
  }
  //
  // Find the smallest range that does not span two non-overlapping MTRRs
  //
  for (Index = 0; Index < FirmwareVariableMtrrCount; Index++) {
    if ( VariableMtrr[Index].Valid 
      ...
      DEBUG((DEBUG_CACHE, "%a():%d MTRR[%d] %lx..%lx %x ?= %x %lx..%lx\n", 
             __FUNCTION__, __LINE__, Index,VariableMtrr[Index].BaseAddress,
             (VariableMtrr[Index].BaseAddress + VariableMtrr[Index].Length - 1), 
             VariableMtrr[Index].Type, Attribute, Start, End));
      if (Start < VariableMtrr[Index].BaseAddress) {
        if ( CheckMemoryAttributeMapped ( 
              FirmwareVariableMtrrCount, 
              Attribute,
              Start,
              (VariableMtrr[Index].BaseAddress-1),
              VariableMtrr,
              MtrrSetting)) {
          Start = VariableMtrr[Index].BaseAddress;
          ...
          //
          // this part of the range has a different cache attribute ending the serach
          //
          ConflictingAttributeFound = TRUE;
          break;
        }
      }
      if (End > (VariableMtrr[Index].BaseAddress + VariableMtrr[Index].Length - 1)) {
        if ( CheckMemoryAttributeMapped ( 
              FirmwareVariableMtrrCount, 
              Attribute,
              ...
              VariableMtrr,
              MtrrSetting)) {
          End = VariableMtrr[Index].BaseAddress + VariableMtrr[Index].Length - 1;

        } else {
          //
          // this part of the range has a different cache attribute ending the search
          //
          ConflictingAttributeFound = TRUE;
          break;
        }
  ...
  if (!ConflictingAttributeFound) {
    //
    // Start..End is a range that does not span MTRRs so the attribute is common
    //
    ConflictingAttributeFound = (Attribute != MtrrGetMemoryAttributeByAddressWorker (MtrrSetting, Start));
//    DEBUG((DEBUG_CACHE, "%a():%d %lx..%lx %x?=%x   %d\n", __FUNCTION__, __LINE__, Start, End, Attribute, MtrrGetMemoryAttribute(Start), ConflictingAttributeFound ));
  }

  return (!ConflictingAttributeFound);
}
//
...
// OVERIDE_HSD_5372585_BEGIN
//
  Mapped = CheckMemoryAttributeMapped (
              FirmwareVariableMtrrCount,
              MemoryType,
              BaseAddress,
  ...
  if (Mapped) {
    DEBUG((DEBUG_CACHE, "%a() Mapped - %lx..%lx %lx\n", __FUNCTION__, BaseAddress, (BaseAddress + Length - 1), MemoryType));
    Status = RETURN_SUCCESS;
    goto Done;
  }
//
...
//
  ...
  if ((UsedMtrr + MtrrNumber) > FirmwareVariableMtrrCount) {
    DEBUG((DEBUG_CACHE, "%a() Not enough MTRRs to map (UsedMtrr:%d + MtrrNumber:%d):%d > FirmwareVariableMtrrCount:%d)\n", __FUNCTION__, UsedMtrr, MtrrNumber, (UsedMtrr + MtrrNumber), FirmwareVariableMtrrCount));
